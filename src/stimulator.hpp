...
//        ans.rd=gr(11,7,so);
    ...
    class queue{//带头节点的队列
        struct node{
              ...
              node *nxt=nullptr;
              node *pre= nullptr;
              node(K v,node *p,node *q):value(v),nxt(p),pre(q){}
              node(){}
        };
        node *head;
        node *rear;
    ...
    public:
        ...
        int in(K value){
            node *p=new node(value,nullptr,rear);
            rear->nxt=p;
            p->pre=rear;
            rear=rear->nxt;
            return ++num;
        ...
        struct RSnode{
            enum OPT opt;
            u32 Vj=0,Vk=0,Qj=0,Qk=0;
            u32 Reorder=0;
            u32 imm;
        }Rs[1000];//Reservation Station
//        struct Reg_rename{
//            u32 value;
//            u32 Reorder=0;
//        }Rename[32];//Rename table
        //-------line-------
        struct Line_to_alu{
            IDResult opt;
        }line_to_alu;
        ...
        void EX(IDResult p){
            ...
            if(p.opt>=BEQ&&p.opt<=BGEU){
            ...
            else if(p.opt<=SW&&p.opt>=LB){
                LSBnode node;
                if(p.opt>=LB&&p.opt<=LHU){
                    node.isload=1;
                    node.rd=rd;
                }
                int rename=lsb.in(node);
                Rename[rd]=rename;
            }
            else if(p.opt>=LUI){
                ROBnode node;
                node.rd=rd;
                node.pc=pc;
                int rename=rob.in(node);
                Rename[rd]=rename;
            }
            else if(p.opt==HALT)return;
            switch(p.opt){
                case LB:
